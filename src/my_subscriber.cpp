...
{
  public:
    ImageProcessor(bool isCalibrationMode):it(nh), sizeColor(960, 540){
      color_mat = Mat(sizeColor, CV_8UC3);
      depth_mat = Mat(sizeColor, CV_16UC1);

      sub = it.subscribe("camera/image", 1,&ImageProcessor::imageCallback,this);
      sub2 = it.subscribe("camera/depth_image",1,&ImageProcessor::depthimageCallback,this);
  ...
  private:
    ...
    Vec3d marker_5_tvecs_sum;
    Vec3d marker_5_rvecs_sum;
    int marker_5_sum_count;
...
void ImageProcessor::depthimageCallback(const sensor_msgs::ImageConstPtr& msgdepth)
{
	...
	try
    {
      vector<int> markerfiveids(1,5);
    	...
    	depth_mat = cv_bridge::toCvShare(msgdepth, "mono16")->image;
...
void ImageProcessor::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    try
    {
      ...
      if(!color_mat.empty() && !depth_mat.empty())
      {
        ...
        if(!key_points.empty())
          drawKeyPoints(displyImg, key_points);
      }
        ...
        for(int i = 0; i<ids.size(); i++)
          {
             ...
             if (ids[i] == 5)
             {
                 if(marker_5_sum_count == 0){
                     marker_5_rvecs_sum = rvecs[i];
                     marker_5_tvecs_sum = tvecs[i];
                     marker_5_sum_count = 1;
                 }
                 Vec3d t_diff = marker_5_tvecs_sum / marker_5_sum_count - tvecs[i];
                 Vec3d r_diff = marker_5_rvecs_sum / marker_5_sum_count - rvecs[i];
                 if ((norm(t_diff) < 0.03 && norm(r_diff) < 0.1 )){
                   if(marker_5_sum_count < 100){
                     marker_5_rvecs_sum += rvecs[i];
                     marker_5_tvecs_sum += tvecs[i];
                     marker_5_sum_count ++;
                   }
                   else{
                   tvecs[i] = marker_5_tvecs_sum / marker_5_sum_count;
                   rvecs[i] = marker_5_rvecs_sum / marker_5_sum_count;
                   }
                 }
                 else{
                   marker_5_rvecs_sum = rvecs[i];
...
int main(int argc, char** argv)
{
  ...
  ImageProcessor img_processor(isCalibrationMode);
